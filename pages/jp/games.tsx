...
class GamesView extends PageBaseClass<BaseProps, States> {
    ...
    renderFilters() {
        return <div style={{ display: "flex", flexDirection: "row", alignItems: "center", marginLeft: 8, marginRight: 8, flexWrap: "wrap", borderColor: "silver", borderWidth: 1, borderStyle: "solid", borderRadius: 15, padding: 8 }}>
            ...
            <TextField label="場所" variant="outlined" onChange={e => this.setState({ location: e.target.value }, () => this.fetchSearchGames())} value={this.state.location} />
            <TextField label="レベル" variant="outlined" onChange={e => this.setState({ level: parseInt(e.target.value) }, () => this.fetchSearchGames())} value={this.state.level} select style={{ marginLeft: 8 }}>
            ...
            <TextField label="この時間から" variant="outlined" onChange={e => this.setState({ time: e.target.value }, () => this.fetchSearchGames())} defaultValue={this.state.time} type="time" style={{ width: 100, marginLeft: 8 }} />
        </div>
    }
        ...
        return (
            <div style={{ paddingTop: 16 }}>
                {(this.state.user) ? <OrganizeForm show={this.state.showPostDialog} uid={this.state.user.id} posted={() => {
                    ...
                    this.showSnackSuccessMsg("ゲームの投稿に成功しました")
                    this.fetchTodaysGames()
                    this.fetchGamesOfTheWeek()
                ...
                <div style={{ display: "flex", flexDirection: "row", alignItems: "center", flexWrap: "nowrap" }}>
                    ...
                    <TextField label="ゲームを探す..." variant="outlined" className={this.styles.formTextField} onChange={e => {
