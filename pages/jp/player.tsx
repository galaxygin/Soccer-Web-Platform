...
export default function PlayerView({ metadata }: props) {
    ...
    return <PageBase content={content()} header={<Header title={(metadata) ? metadata.name : "プレイヤー名の取得に失敗しました"} description={(metadata?.is_private) ? "非公開か、説明の取得に失敗しました" : metadata?.bio} thumbnail_url={metadata?.thumbnail_url} url={""} />} region={"jp"} onStateChanged={user => {
        setUser(user)
    }} />
}
