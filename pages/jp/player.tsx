            ...
            if (this.state.user)
                return (
                    <div style={{ display: "flex", flexDirection: "column", height: this.state.height! - 115 }}>
                        ...
                        >
                            <MenuItem onClick={() => this.setState({ thumbAnchorEl: null, changeThumb: true })}>サムネ画像を変更</MenuItem>
                        </Menu>
                        <Menu
                        ...
                        >
                            <MenuItem onClick={() => this.setState({ headerAnchorEl: null, changeHeader: true })}>ヘッダー画像を変更</MenuItem>
                        </Menu>
                        <Dialog open={this.state.changeThumb} onClose={() => this.setState({ newThumb: undefined, changeThumb: false })} fullWidth>
                            ...
                            <DialogActions style={{ backgroundColor: 'white', marginTop: 32 }}>
                                <Button variant="outlined" onClick={() => this.setState({ changeThumb: false })}>閉じる</Button>
                                <div style={{ flexGrow: 1 }} />
                                <Button variant="outlined" onClick={() => this.setState({ changeThumb: false })}>完了</Button>
                            </DialogActions>
                        </Dialog>
                        <Dialog open={this.state.changeHeader} onClose={() => this.setState({ newHeader: undefined, changeHeader: false })} fullWidth>
                            ...
                            <DialogActions style={{ backgroundColor: 'white', marginTop: 32 }}>
                                <Button variant="outlined" onClick={() => this.setState({ changeHeader: false })}>閉じる</Button>
                                <div style={{ flexGrow: 1 }} />
                                <Button variant="outlined" onClick={() => this.setState({ changeHeader: false })}>完了</Button>
                            </DialogActions>
                        </Dialog>
                        ...
                        <div style={{ backgroundColor: defaultTheme, height: "100%", borderColor: "white", borderWidth: 1, borderStyle: "solid" }}>
                            {(this.state.editingProfile) ? <div style={{ display: "flex", padding: 16, flexDirection: "column" }}>
                                {this.editButton()}
                                <TextField label="プロフィール" variant="outlined" className={this.styles.formTextField} onChange={e => this.setState({ visibility: e.target.value })} defaultValue={this.state.visibility} select>
                                    <MenuItem key={"public"} value={"public"}>公開</MenuItem>
                                    <MenuItem key={"private"} value={"private"}>非公開</MenuItem>
                                </TextField>
                                <TextField label="表示名" variant="outlined" className={this.styles.formTextField} onChange={e => this.setState({ name: e.target.value })} value={this.state.name} />
                                <TextField label="ポジション" variant="outlined" className={this.styles.formTextField} onChange={e => this.setState({ position: e.target.value })} defaultValue={this.state.position} select>
                                ...
                                <TextField label="地元" variant="outlined" className={this.styles.formTextField} onChange={e => this.setState({ localArea: e.target.value })} value={this.state.localArea} />
                                <TextField label="説明" variant="outlined" className={this.styles.formTextField} onChange={e => this.setState({ bio: e.target.value })} value={this.state.bio} fullWidth multiline minRows={4} />
                                ...
                                <div style={{ display: "flex", flexDirection: "row", alignItems: "center", width: "100%", height: 100 }}>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                                        ...
                                        <Typography style={{ color: darkerTextColor, marginTop: 16 }}>
                                            ポジション: {this.state.player.position}<br />
                                            地元: {this.state.player.local_area}
                                        </Typography>
                                    </div>
                                    <div style={{ flexGrow: 1 }} />
                                    {this.editButton()}
