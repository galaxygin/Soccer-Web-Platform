...
export async function getProfile(uid: string): Promise<Player> {
    const { data, error } = await supabase.from('players').select('*').eq('uid', uid).limit(1)
    if (error)
        throw error
    if (data)
        if (data.length > 0)
...
export async function getSimpleProfile(uid: string): Promise<SimpleProfile> {
    const { data, error } = await supabase.from('players').select('name, thumbnail_url, position, is_private').eq('uid', uid).limit(1)
    if (error)
        throw error
    if (data)
        if (data.length > 0)
...
export async function checkUserRegisteredAsPlayer(uid: string): Promise<boolean> {
    const { data, error } = await supabase.from('players').select('uid').eq('uid', uid).limit(1)
    if (error)
        throw error
    if (data)
        if (data.length > 0)
...
export async function uploadHeader(uid: string, image: File): Promise<string> {
    let { data, error } = await supabase.storage.from('userdata/headers').upload(uid + '.jpg', image, { upsert: true })
    if (error)
        throw error
    return data?.Key!
}

export function getHeaderDownloadURL(uid: string): string {
    const { publicURL, error } = supabase.storage.from('userdata/headers').getPublicUrl(uid + '.jpg')
    if (error)
        throw error
    return publicURL!
}

export async function updateHeaderURL(uid: string, url: string) {
    const { data, error } = await supabase.from('players').update({ 'header_url': url }).eq('uid', uid)
    if (error)
        throw error
    return data
}